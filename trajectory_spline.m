...
segment_start = 1;
xq = zeros(sum(num_points_per_segment),1);
for segment = 1:num_segments
    segment_end = sum(num_points_per_segment(1:segment));
    interpolated_times = linspace(times(segment),times(segment+1),num_points_per_segment(segment));
    xq(segment_start:segment_end,1) = interpolated_times;
    segment_start = segment_end+1;
end

trajectory = zeros(num_joints, sum(num_points_per_segment));
trajectory(:,:) = spline(times, [zeros(size(waypoints(:,1))) waypoints zeros(size(waypoints(:,1)))], xq);
