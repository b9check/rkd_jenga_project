...
for segment = 1:num_segments
    ...
    T = times(segment+1) - times(segment);

    for i = 1:num_joints
        d = waypoints(i,segment+1) - waypoints(i,segment);
        vm(i,1) = d/(T - ramp_time);
    end

    % RAMP UP  
    ramp_up_points = zeros(num_joints,num_ramp_points-1);
    ramp_up_points(:,1) = waypoints(:,segment);
    for i = 1:num_joints
        for j = 2:num_ramp_points
            v = vm(i,1)*((j-1)/num_ramp_points);
            ramp_up_points(i,j) = ramp_up_points(i,j-1) + v/frequency;
        end
    end
    trajectory(:, segment_start_point : segment_start_point + num_ramp_points - 1) = ramp_up_points;
    
    %CONSTANT
    num_const_points = segment_end_point - segment_start_point - 2*num_ramp_points + 1;
    const_time = num_const_points/frequency;
    const_points = zeros(num_joints,num_const_points);

    for i = 1:num_joints
        v = vm(i,1);
        p0 = ramp_up_points(i,end);
        p1 = p0 + v*const_time;
        const_points(i,:) = linspace(p0,p1,num_const_points);
    end
    trajectory(:, segment_start_point + num_ramp_points : segment_end_point - num_ramp_points) = const_points; % constant velocity
    

    ramp_down_points = zeros(num_joints,num_ramp_points);
    ramp_down_points(:,1) = trajectory(:, segment_end_point - num_ramp_points);
    for i = 1:num_joints
        for j = 2:num_ramp_points
                v = vm(i,1) - vm(i,1)*(j/num_ramp_points);
                ramp_down_points(i,j) = ramp_down_points(i,j-1) + v/frequency;
        end
    end

    trajectory(:, segment_end_point - num_ramp_points + 1 : segment_end_point) = ramp_down_points;                % ramp down
